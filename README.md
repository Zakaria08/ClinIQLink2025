...
- **Integration:**  
  - Use a different LLM to refine and optimize the prompts.
  - Combine the optimized prompt with own prompt template to improve the solutions and still keep the answer structure.
- **Process:**  
  - Calling a GPT Wrapper which improves Prompts
  - Take the best part of the prompt and concatenate the improved prompt with the question and format of the answer.
-  **Potential Challenges:**
      - We could not use the whole prompt which was generated by the GPT Wrapper. Due to not receiving the format of the answers we wanted.
      - Dependency on the GPT Wrapper. What to do if the wrapper Website crashes?
